...
struct BoundVarReplacer<'a, 'gcx: 'a + 'tcx, 'tcx: 'a> {
    ...
    fld_c: &'a mut (dyn FnMut(ty::BoundVar, ty::Ty<'tcx>) -> &'tcx ty::LazyConst<'tcx> + 'a),
}
...
impl<'a, 'gcx, 'tcx> BoundVarReplacer<'a, 'gcx, 'tcx> {
    fn new<F, G, H>(
        ...
        where F: FnMut(ty::BoundRegion) -> ty::Region<'tcx>,
              ...
              H: FnMut(ty::BoundVar, ty::Ty<'tcx>) -> &'tcx ty::LazyConst<'tcx>,
    {
        BoundVarReplacer {
            tcx,
            current_index: ty::INNERMOST,
...
impl<'tcx> TypeVisitor<'tcx> for HasTypeFlagsVisitor {
    ...
    fn visit_const(&mut self, ct: &'tcx ty::LazyConst<'tcx>) -> bool {
        let flags = ct.type_flags();
        debug!("HasTypeFlagsVisitor: ct={:?} ct.flags={:?} self.flags={:?}", ct, flags, self.flags);
