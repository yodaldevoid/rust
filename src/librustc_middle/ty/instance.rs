...
pub enum InstanceDef<'tcx> {
    /// An item, the second field is the `DefId` of the corresponding generic parameter
    /// for const arguments.
    ///
    /// Unlike most other uses of a `param_did`, this may be `None` even if a
    /// generic parameter exists. To always get the correct `WithOptParam`,
    /// `InstanceDef::with_opt_param(tcx)` should be used.
    Item(DefId, Option<DefId>),
